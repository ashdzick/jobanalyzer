...
**Resumes/** — Drop your resume PDF(s) here. If you have multiple versions add them all. The system reads the job description first and recommends which resume is the best fit before running the match.
...
1. On first launch, the system will run a short preferences quiz — six questions, one at a time. Answer them and it produces your `preferences.md`. Every session after that, it loads silently.
2. Paste a job description when prompted. The system checks it against your preferences, then runs the full resume match.
3. Update **Analyses/index.md** as your application status changes — Interested, Applied, Interviewing, Offer, Passed, or Declined.
...
Every conversation you start inside the Project will have your resume and preferences loaded automatically. Skip the Analyses folder since those files change with every session. Download individual analysis files from the chat and store them wherever works for you.

### Use This in a Cowork Session

This is the most seamless setup. Claude reads your files directly from your computer, so analyses save automatically and your index stays in sync without any manual steps.
...
Claude will read your resume and preferences directly, run the analysis, save the output to your Analyses folder, and update the index.
